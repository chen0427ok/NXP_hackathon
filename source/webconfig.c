...
static int CGI_HandleSensorData(HTTPSRV_CGI_REQ_STRUCT *param);
...
// 前向宣告
void format_and_print_scan_result(const char *scan_result);
/*******************************************************************************
 * Definitions
 ******************************************************************************/
...
const HTTPSRV_CGI_LINK_STRUCT cgi_lnk_tbl[] = {
	...
	{"sensorData", CGI_HandleSensorData},  // 新增這一行
...
static int CGI_HandleSensorData(HTTPSRV_CGI_REQ_STRUCT *param)
{
    ...
    /* 模擬溫度、空氣濕度、土壤濕度數據 */
    int temperature = rand() % 31 + 10;  // 模擬 10~40°C 的溫度
	int humidity = rand() % 31 + 10;     //
	int soil_moisture = rand() % 31 + 10; //

    /* 準備 JSON 回應 */
    char buffer[256] = {0};
    snprintf(buffer, sizeof(buffer), "{\"sensorData\":{\"temperature\":%d,\"humidity\":%d,\"soilMoisture\":%d}}",
...
static void main_task(void *arg)
{
    ...
    char *ssids = WPL_Scan();  // Scan for Wi-Fi networks
    ...
    /* ------- Prompt the user to connect to a network ------- */
       ...
       scanf("%s", user_ssid);  // Get user input for SSID

       PRINTF("\n[i] Please enter the password for the network: ");
       fflush(stdout);  // 強制刷新，確保輸出即時顯示
       char user_password[WPL_WIFI_PASSWORD_LENGTH] = {0};
       scanf("%s", user_password);  // Get user input for password

       /* Attempt to join the selected network */
       PRINTF("\n[i] Attempting to connect to SSID: %s\r\n", user_ssid);
       PRINTF("\n[i] Attempting to connect to password: %s\r\n", user_password);
       fflush(stdout);  // 確保輸出即時顯示
       result = WPL_AddNetworkWithSecurity(user_ssid, user_password, WIFI_NETWORK_LABEL, WPL_SECURITY_WPA2);
       if (result == WPLRET_SUCCESS)
       {
           result = WPL_Join(WIFI_NETWORK_LABEL);
           if (result == WPLRET_SUCCESS)
...
static uint32_t SetBoardToClient()
{
    ...
    if (!g_BoardState.connected)
    {
        ...
        else if (strstr(g_BoardState.security, "WPA2"))
        {
            result = WPL_AddNetworkWithSecurity(g_BoardState.ssid, g_BoardState.password, WIFI_NETWORK_LABEL, WPL_SECURITY_WPA2);
        }
        else
        {
        	printf("else\n");
...
void format_and_print_scan_result(const char *scan_result)
{
    /* 假設掃描結果是 JSON 格式的，可以使用 sscanf 或手動解析 */
    /* 這裡假設掃描結果類似於以下格式：
     * {"networks":[{"ssid":"wifi-5F-4","bssid":"74:93:DA:30:43:5F","signal":"-90dBm","channel":1,"security":"WPA2"},
     * {"ssid":"meowMe","bssid":"5E:D7:87:F2:96:96","signal":"-83dBm","channel":149,"security":"WPA2"}]}
     */

    const char *ptr = scan_result;
    while ((ptr = strstr(ptr, "\"ssid\":")) != NULL)
    {
        char ssid[WPL_WIFI_SSID_LENGTH] = {0};
        char bssid[18] = {0};  // BSSID 格式為 MAC 地址
        char signal[8] = {0};
        int channel = 0;
        char security[16] = {0};

        sscanf(ptr, "\"ssid\":\"%[^\"]\",\"bssid\":\"%[^\"]\",\"signal\":\"%[^\"]\",\"channel\":%d,\"security\":\"%[^\"]\"",
               ssid, bssid, signal, &channel, security);

        PRINTF("SSID: %s, BSSID: %s, Signal: %s, Channel: %d, Security: %s\r\n",
               ssid, bssid, signal, channel, security);

        ptr++;  // Move pointer forward to find the next SSID
    }
}
